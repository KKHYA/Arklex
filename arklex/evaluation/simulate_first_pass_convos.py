...
def check_goal_completion_enhanced(goal: str, convo: List[Dict[str, Any]], client: Any) -> bool:
    """
    Enhanced user goal completion evaluation with detailed scoring system.
    Uses a 100-point scoring system to better assess goal achievement stability.
    """
...
Evaluate whether the user's goal was successfully completed using this 100-point scoring system:

**Problem Identification & Understanding (30 points)**:
- Did the chatbot correctly understand the user's main goal or need? (15 points)
- Was the user's specific situation properly acknowledged? (15 points)

**Solution Completeness (30 points)**:
- Were concrete, actionable solutions provided to address the user's goal? (20 points)
- Was sufficient information given to help the user proceed? (10 points)

**Information Quality & Usefulness (25 points)**:
- Were the responses relevant and accurate to the user's specific goal? (15 points)
- Did the chatbot provide practical, usable information? (10 points)

**Goal Achievement Assessment (15 points)**:
- Would the user reasonably consider their goal accomplished after this conversation? (10 points)
- Does the user have clear next steps or resolution? (5 points)

Calculate the total score out of 100 points. The user's goal is considered successfully completed if the score is 70 points or higher.

Output format:
SCORE: [total points]
SUCCESS: [True/False based on >= 85 points]
ANALYSIS: [Brief explanation of scoring]

Only output the SCORE, SUCCESS, and ANALYSIS lines."""
    ...
    try:
        ...
        success_line = next((line for line in lines if line.startswith('SUCCESS:')), None)
        analysis_line = next((line for line in lines if line.startswith('ANALYSIS:')), None)
        
        if score_line and success_line and analysis_line:
            score = float(score_line.split(':', 1)[1].strip())
            success_result = success_line.split(':', 1)[1].strip().lower() == 'true'
            analysis = analysis_line.split(':', 1)[1].strip()
            return score, success_result, analysis
...
def check_goal_completion(goal: str, convo: List[Dict[str, Any]], client: Any) -> bool:
    """
    Original simple goal completion check (kept for backward compatibility).
    """
    convo_str: str = format_chat_history_str(flip_hist_content_only(convo[2:]))
    prompt: str = f"Here is a conversation between a user and a customer service chatbot assistant:\n{convo_str}\n\nThe user's goal is the following: {goal}\nOutput False if the user needs to learn more information regarding their goal. Output True otherwise. Only output True or False and nothing else."
    output: str = chatgpt_chatbot([{"role": "user", "content": prompt}], client)
    return output == "True"
...
def conversation(
    ...
    for i in range(synthetic_data_params["max_turns"]):
        ...
        try:
            score, success, analysis = check_goal_completion_enhanced(goal, history.copy(), env_config["client"])
        except Exception as e:
            print("Error checking goal completion:", e)
            traceback.print_exc()
            score = None
            success = check_goal_completion(goal, history.copy(), env_config["client"])
            analysis = None
        if i > 2 and success:
            goal_completion = True
            # history.append({'goal_completion': True})
            break

    # if not history[-1].get('goal_completion', False):
    #     history.append({'goal_completion': False})
    return history, goal_completion, score, analysis
...
def generate_conversations(
    ...
    def worker(input_combo):
        convo, goal_completion, score, analysis = conversation(
            model_api,
            input_combo["profile"],
            input_combo["goal"],
            input_combo["attr"],

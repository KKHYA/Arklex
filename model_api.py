...
def predict(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    logger.info("Received eval/chat request")
    try:
        ...
        logger.debug(f"History length: {len(history)}, Workers count: {len(workers)}, Tools count: {len(tools)}")
        ...
        logger.info(f"Bot response generated successfully, answer length: {len(answer)}")
        logger.debug(f"Updated parameters: {params}")

        logger.info("Eval/chat request processed successfully")
        return {"answer": answer, "parameters": params}
        
    except Exception as e:
        logger.error(f"Error processing eval/chat request: {str(e)}")
        raise
...
if __name__ == "__main__":
    ...
    log_level: int = getattr(logging, args.log_level.upper(), logging.DEBUG)
    log_timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    logger: logging.Logger = init_logger(
        log_level=log_level,
        filename=os.path.join(os.path.dirname(__file__), "logs", f"arklex_api_{log_timestamp}.log"),
    )

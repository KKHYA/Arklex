...
from arklex.env.tools.car_tools.build_car_database import build_car_database
...
logger = init_logger(
    ...
    filename=os.path.join(os.path.dirname(__file__), "logs", f"arklex_{log_timestamp}.log"),
)
load_dotenv()
...
def init_worker(args: argparse.Namespace) -> None:
    ...
    - CarDatabaseWorker: For automotive database operations
    ...
    logger.info(f"Found workers: {worker_names}")
    ...
    # Initialize CarDatabaseWorker for automotive applications
    elif "CarDatabaseWorker" in worker_names:
        db_path = os.path.join(args.output_dir, "car_advisor_db.sqlite")
        if not os.path.exists(db_path):
            logger.info("Initializing CarDatabaseWorker...")
            build_car_database(args.output_dir)
        else:
            logger.info("CarDatabaseWorker already initialized")
